    ...
    >
      <ExamplePreview class="relative flex w-full items-center justify-center h-[320px]">
        <TheList class="w-fit touch-none select-none" :columns="4" :rows="3">
          <TheRow class="group" :index="0">
            <TheEmoji
              :emoji="{ emoji: '😊', label: 'Smiling face with smiling eyes' }"
              :index="0"
            />
            <TheEmoji :emoji="{ emoji: '🎀', label: 'Ribbon' }" :index="1" />
            <TheEmoji :emoji="{ emoji: '🥑', label: 'Avocado' }" :index="2" />
            <TheEmoji :emoji="{ emoji: '🌈', label: 'Rainbow' }" :index="3" />
          </TheRow>
          <TheRow class="group" :index="1">
            <TheEmoji :emoji="{ emoji: '🖤', label: 'Black heart' }" :index="0" />
            <TheEmoji :emoji="{ emoji: '❤️', label: 'Red heart' }" :index="1" />
            <TheEmoji :emoji="{ emoji: '🧡', label: 'Orange heart' }" :index="2" />
            <TheEmoji :emoji="{ emoji: '💛', label: 'Yellow heart' }" :index="3" />
          </TheRow>
          <TheRow class="group" :index="2">
            <TheEmoji :emoji="{ emoji: '🤍', label: 'White heart' }" :index="0" />
            <TheEmoji :emoji="{ emoji: '💚', label: 'Green heart' }" :index="1" />
            <TheEmoji :emoji="{ emoji: '💙', label: 'Blue heart' }" :index="2" />
            <TheEmoji :emoji="{ emoji: '💜', label: 'Purple heart' }" :index="3" />
          </TheRow>
        </TheList>
